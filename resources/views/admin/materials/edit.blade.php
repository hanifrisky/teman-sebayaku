...
<div class="max-w-3xl mx-auto space-y-6 animate-fade-in">
    <div class="card p-6">
        ...
        <form action="{{ route('admin.tribes.materials.update', [$tribe, $material]) }}" method="POST" class="space-y-6">
            ...
            @php
                $qW = $material->questions->where('category', 'W')->first();
                $qD = $material->questions->where('category', 'D')->first();
                $qE = $material->questions->where('category', 'E')->first();
                $qP = $material->questions->where('category', 'P')->first();
            @endphp
            <div class="space-y-4 pt-4 border-t border-slate-100">
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                        ...
                        <textarea name="questions[0][text]" required rows="2" class="form-input bg-white text-xs" placeholder="Contoh: Apa keinginan atau tujuan yang ingin kamu capai dalam hidup saat ini?">{{ old('questions.0.text', $qW ? $qW->text : '') }}</textarea>
                    </div>
                    ...
                    <div class="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                        ...
                        <textarea name="questions[1][text]" required rows="2" class="form-input bg-white text-xs" placeholder="Contoh: Langkah-langkah nyata apa yang sedang kamu lakukan untuk mencapai keinginan tersebut?">{{ old('questions.1.text', $qD ? $qD->text : '') }}</textarea>
                    </div>
                    ...
                    <div class="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                        ...
                        <textarea name="questions[2][text]" required rows="2" class="form-input bg-white text-xs" placeholder="Contoh: Apakah langkah yang kamu lakukan saat ini sudah selaras dengan kearifan lokal kesabaran?">{{ old('questions.2.text', $qE ? $qE->text : '') }}</textarea>
                    </div>
                    ...
                    <div class="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                        ...
                        <textarea name="questions[3][text]" required rows="2" class="form-input bg-white text-xs" placeholder="Contoh: Rencana tindakan kecil dan realistis apa yang akan kamu lakukan selanjutnya?">{{ old('questions.3.text', $qP ? $qP->text : '') }}</textarea>
                    </div>
                </div>
            </div>
