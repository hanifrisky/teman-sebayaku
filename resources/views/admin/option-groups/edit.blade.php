...
@section('title', 'Edit Grup Pilihan Jawaban')
@section('page-title', 'Edit Grup Pilihan')

@section('content')
<div class="max-w-2xl mx-auto space-y-6 animate-fade-in">
    <div class="card p-6">
        <div class="flex items-center justify-between mb-6">
            <h3 class="font-bold text-slate-800 text-lg">Edit Grup Pilihan</h3>
        ...
        <form action="{{ route('admin.option-groups.update', $optionGroup) }}" method="POST" class="space-y-6" x-data="{ 
            items: {{ json_encode($optionGroup->items->sortBy('order')->values()->map(function($item) { return ['label' => $item->label, 'score' => $item->score]; })) }},
            addItem() {
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center gap-3 justify-end">
                ...
                <button type="submit" class="btn-primary">Perbarui Grup</button>
            </div>
        </form>
    </div>
</div>
